...
function computeItemTotals(item: Omit<CartItem, 'addons_total' | 'line_subtotal' | 'discount_amount' | 'line_total'> & { addons: CartAddon[] }, scPct: number, pwdPct: number): CartItem {
...
interface CartStore {
  cart: CartState
  scPct: number  // from settings
  pwdPct: number // from settings
  setDiscountPcts: (sc: number, pwd: number) => void

  addItem: (item: {
    item_id: string; item_name: string; size_name?: string
    base_price: number; addons?: CartAddon[]
  }) => void
...
export const useCartStore = create<CartStore>()((set, get) => ({
  ...
  addItem: ({ item_id, item_name, size_name, base_price, addons = [] }) => {
    set((state) => {
      const key = cartKey(item_id, size_name, addons.map(a => a.addon_id))
      const existing = state.cart.items.find(i => i.cart_key === key)
      if (existing) {
      ...
      const raw = {
        cart_key: key, item_id, item_name, size_name,
        base_price, qty: 1, discount_type: null as DiscountType,
        discount_pct: 0, addons,
      }
      return { cart: { ...state.cart, items: [...state.cart.items, computeItemTotals(raw, state.scPct, state.pwdPct)] } }
    })
  },

  removeItem: (cart_key) => {
    set(s => ({ cart: { ...s.cart, items: s.cart.items.filter(i => i.cart_key !== cart_key) } }))
  },

  updateQty: (cart_key, delta) => {
    set((state) => {
      const items = state.cart.items
        .map(i => i.cart_key === cart_key
  ...
  clearCart: () => set(s => ({
    cart: {
      items: [], order_type: 'dine_in', note: '',
      idempotency_key: crypto.randomUUID(),
    },
  })),

  resetIdempotencyKey: () => set(s => ({
    cart: { ...s.cart, idempotency_key: crypto.randomUUID() },
  })),
