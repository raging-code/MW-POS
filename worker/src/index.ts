...
const saleItems = sqliteTable('sale_items', {
  ...
  discount_type:   text('discount_type', { enum: ['sc', 'pwd', 'p15', 'p100', 'p10'] }),
...
app.post('/api/shifts/:id/cash-drop', async (c) => {
  ...
  const id = uid()
  await db.insert(cashDrops).values({ id, shift_id, user_id: actor.id, amount: body.amount, reason: body.reason, created_at: nowISO(), type: 'drop' })
...
type CheckoutItem = {
  ...
  base_price: number
  qty: number
  discount_type?: 'sc' | 'pwd' | 'p15' | 'p100' | 'p10'
  discount_pct: number
...
app.post('/api/sales', async (c) => {
  ...
  for (const item of body.items) {
    ...
    const itemBase = (item.base_price * item.qty) + addonsTotal
    ...
    saleItemRows.push({
      ...
      discount_pct: (item.discount_type === 'sc' || item.discount_type === 'pwd' || item.discount_type === 'p15' || item.discount_type === 'p100' || item.discount_type === 'p10') ? 0 : item.discount_pct,
