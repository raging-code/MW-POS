...
// .wrangler/tmp/bundle-oBGfN9/checked-fetch.js
...
var pinAttempts = sqliteTable("pin_attempts", {
...
var cashDrops = sqliteTable("cash_drops", {
  id: text("id").primaryKey(),
...
var heldOrders = sqliteTable("held_orders", {
  id: text("id").primaryKey(),
...
var receiptCounters = sqliteTable("receipt_counters", {
...
var saleItemAddons = sqliteTable("sale_item_addons", {
  id: text("id").primaryKey(),
...
var salePayments = sqliteTable("sale_payments", {
  id: text("id").primaryKey(),
  sale_id: text("sale_id").notNull().references(() => sales.id),
...
var timeLogs = sqliteTable("time_logs", {
  id: text("id").primaryKey(),
  user_id: text("user_id").notNull().references(() => users.id),
  clock_in: text("clock_in").notNull(),
  clock_out: text("clock_out"),
  edited_by: text("edited_by").references(() => users.id),
...
var inventoryTransactions = sqliteTable("inventory_transactions", {
  id: text("id").primaryKey(),
...
async function generateReceiptNumber(d1) {
...
async function createAuditLog(db, userId, action, entityType, entityId, oldValue, newValue, reason) {
  await db.insert(auditLogs).values({
    id: uid(),
    user_id: userId,
    action,
    entity_type: entityType,
    entity_id: entityId ?? void 0,
    old_value: oldValue != null ? JSON.stringify(oldValue) : void 0,
    new_value: newValue != null ? JSON.stringify(newValue) : void 0,
    reason: reason ?? void 0,
    created_at: nowISO()
  });
}
__name(createAuditLog, "createAuditLog");
async function checkPinLock(db, identifier) {
  const rec = await db.select().from(pinAttempts).where(eq(pinAttempts.identifier, identifier)).get();
  if (!rec) return null;
  if (rec.locked_until && new Date(rec.locked_until) > /* @__PURE__ */ new Date()) {
    return `Too many attempts. Try again after ${rec.locked_until}`;
  }
  return null;
}
__name(checkPinLock, "checkPinLock");
async function recordPinFail(db, identifier) {
  const rec = await db.select().from(pinAttempts).where(eq(pinAttempts.identifier, identifier)).get();
  const attempts = (rec?.attempts ?? 0) + 1;
  const locked_until = attempts >= 5 ? addHours(0.25) : null;
  await db.insert(pinAttempts).values({
    identifier,
    attempts,
    locked_until: locked_until ?? void 0,
    updated_at: nowISO()
  }).onConflictDoUpdate({
    target: pinAttempts.identifier,
    set: { attempts, locked_until: locked_until ?? void 0, updated_at: nowISO() }
  });
}
__name(recordPinFail, "recordPinFail");
async function clearPinFail(db, identifier) {
  await db.update(pinAttempts).set({ attempts: 0, locked_until: void 0, updated_at: nowISO() }).where(eq(pinAttempts.identifier, identifier));
}
__name(clearPinFail, "clearPinFail");
var app = new Hono2();
...
app.use("/api/*", async (c, next) => {
  ...
  const session = await db.select({ user_id: sessions.user_id, expires_at: sessions.expires_at }).from(sessions).where(eq(sessions.token, tok)).get();
  if (!session || new Date(session.expires_at) < /* @__PURE__ */ new Date()) {
    return jsonErr("Session expired", 401);
  }
  const user = await db.select({ id: users.id, name: users.name, role: users.role, is_active: users.is_active }).from(users).where(eq(users.id, session.user_id)).get();
...
app.get("/api/auth/users", async (c) => {
  const db = drizzle(c.env.DB);
  const list = await db.select({ id: users.id, name: users.name, role: users.role }).from(users).where(eq(users.is_active, true)).orderBy(asc(users.name));
  return jsonOk(list);
});
app.post("/api/auth/login", async (c) => {
  ...
  const user = await db.select().from(users).where(and(eq(users.id, body.user_id), eq(users.is_active, true))).get();
...
app.post("/api/auth/verify-pin", async (c) => {
  ...
  return jsonOk({ verified: true, role: user.role });
});
app.get("/api/users", async (c) => {
  const db = c.get("db");
  const list = await db.select({ id: users.id, name: users.name, role: users.role, is_active: users.is_active, created_at: users.created_at }).from(users).orderBy(asc(users.name));
  return jsonOk(list);
});
...
app.get("/api/menu", async (c) => {
  ...
  const menu = cats.map((cat) => ({
    ...cat,
    items: items.filter((i) => i.category_id === cat.id).map((item) => ({
      ...item,
      sizes: sizes.filter((s) => s.item_id === item.id),
      addons: itemAddonsAll.filter((ia) => ia.item_id === item.id).map((ia) => addonsAll.find((a) => a.id === ia.addon_id)).filter(Boolean)
    }))
  }));
...
app.get("/api/shifts/current", async (c) => {
  ...
  const payments = await db.select({ method: salePayments.method, amount: salePayments.amount }).from(salePayments).innerJoin(sales, eq(sales.id, salePayments.sale_id)).where(and(eq(sales.shift_id, shift.id), eq(sales.status, "completed")));
  const totals = {};
...
app.post("/api/shifts", async (c) => {
  ...
  await db.insert(shifts).values({
    id,
    cashier_id: actor.id,
    starting_float: body.starting_float,
    opened_by: actor.id,
    status: "open",
    started_at: nowISO()
  });
...
app.post("/api/sales", async (c) => {
  ...
  const existing = await db.select({ id: sales.id, receipt_number: sales.receipt_number }).from(sales).where(eq(sales.idempotency_key, body.idempotency_key)).get();
  ...
  if (hasCash && body.tendered_amount !== void 0 && body.tendered_amount < cashPayments.reduce((s, p) => s + p.amount, 0)) {
    return jsonErr("Tendered amount less than cash portion");
  }
  const receiptNumber = await generateReceiptNumber(c.env.DB);
  const saleId = uid();
  const change = hasCash && body.tendered_amount != null ? Math.round((body.tendered_amount - total) * 100) / 100 : 0;
  saleItemRows.forEach((r) => {
    r.sale_id = saleId;
  });
...
app.get("/api/sales", async (c) => {
  ...
  let query = db.select({
    id: sales.id,
    receipt_number: sales.receipt_number,
  ...
  if (status) query = query.where(eq(sales.status, status));
...
app.get("/api/sales/:id", async (c) => {
  ...
  const addonList = itemIds.length ? await db.select().from(saleItemAddons).where(sql`sale_item_id IN (${sql.join(itemIds.map((i) => sql`${i}`), sql`,`)})`) : [];
...
app.get("/api/time-logs", async (c) => {
  const actor = c.get("user");
  const db = c.get("db");
  const { user_id, date_from, date_to } = c.req.query();
  const targetId = actor.role === "admin" ? user_id ?? null : actor.id;
  let query = db.select({
    id: timeLogs.id,
    user_id: timeLogs.user_id,
    clock_in: timeLogs.clock_in,
    clock_out: timeLogs.clock_out,
    edited_by: timeLogs.edited_by,
    edit_reason: timeLogs.edit_reason,
    created_at: timeLogs.created_at,
    user_name: users.name
  }).from(timeLogs).innerJoin(users, eq(users.id, timeLogs.user_id)).orderBy(desc(timeLogs.clock_in)).$dynamic();
  if (targetId) query = query.where(eq(timeLogs.user_id, targetId));
  if (date_from) query = query.where(gte(timeLogs.clock_in, date_from));
  if (date_to) query = query.where(lte(timeLogs.clock_in, date_to + "T23:59:59"));
  const list = await query;
  return jsonOk(list);
});
app.post("/api/time-logs/clock-in", async (c) => {
  const actor = c.get("user");
  const db = c.get("db");
  const open = await db.select({ id: timeLogs.id }).from(timeLogs).where(and(eq(timeLogs.user_id, actor.id), isNull(timeLogs.clock_out))).get();
  if (open) return jsonErr("Already clocked in");
  const id = uid();
  await db.insert(timeLogs).values({ id, user_id: actor.id, clock_in: nowISO(), created_at: nowISO() });
  return jsonOk({ id, clock_in: nowISO() });
});
app.post("/api/time-logs/clock-out", async (c) => {
  const actor = c.get("user");
  const db = c.get("db");
  const open = await db.select({ id: timeLogs.id, clock_in: timeLogs.clock_in }).from(timeLogs).where(and(eq(timeLogs.user_id, actor.id), isNull(timeLogs.clock_out))).orderBy(desc(timeLogs.clock_in)).get();
  if (!open) return jsonErr("Not clocked in");
  const clock_out = nowISO();
  await db.update(timeLogs).set({ clock_out }).where(eq(timeLogs.id, open.id));
  return jsonOk({ id: open.id, clock_in: open.clock_in, clock_out });
});
app.put("/api/time-logs/:id", async (c) => {
  const actor = c.get("user");
  if (actor.role !== "admin") return jsonErr("Admin only", 403);
  const db = c.get("db");
  const id = c.req.param("id");
  const body = await c.req.json();
  if (!body.reason) return jsonErr("Reason required");
  const old = await db.select().from(timeLogs).where(eq(timeLogs.id, id)).get();
  if (!old) return jsonErr("Log not found", 404);
  await db.update(timeLogs).set({
    clock_in: body.clock_in ?? old.clock_in,
    clock_out: body.clock_out ?? old.clock_out,
    edited_by: actor.id,
    edit_reason: body.reason
  }).where(eq(timeLogs.id, id));
  await createAuditLog(db, actor.id, "edit_time_log", "time_log", id, old, body, body.reason);
  return jsonOk({ ok: true });
});
app.get("/api/reports/sales", async (c) => {
  ...
  if (saleIds.length) {
    const payments = await db.select().from(salePayments).where(sql`sale_id IN (${sql.join(saleIds.map((i) => sql`${i}`), sql`,`)})`);
    for (const p of payments) {
...
app.get("/api/reports/work-hours", async (c) => {
  const actor = c.get("user");
  if (actor.role !== "admin") return jsonErr("Admin only", 403);
  const db = c.get("db");
  const { date_from, date_to } = c.req.query();
  let query = db.select({
    ...
    clock_out: timeLogs.clock_out,
    user_name: users.name
  }).from(timeLogs).innerJoin(users, eq(users.id, timeLogs.user_id)).$dynamic();
  if (date_from) query = query.where(gte(timeLogs.clock_in, date_from));
  if (date_to) query = query.where(lte(timeLogs.clock_in, date_to + "T23:59:59"));
  const logs = await query;
  const settings = await db.select().from(systemSettings).where(eq(systemSettings.key, "hourly_rate")).get();
  const hourlyRate = parseFloat(settings?.value ?? "80");
  const byUser = {};
  for (const log of logs) {
    if (!log.clock_out) continue;
    const mins = Math.round((new Date(log.clock_out).getTime() - new Date(log.clock_in).getTime()) / 6e4);
    if (!byUser[log.user_id]) byUser[log.user_id] = { user_id: log.user_id, user_name: log.user_name, total_minutes: 0 };
    byUser[log.user_id].total_minutes += mins;
  }
  const summary = Object.values(byUser).map((u) => ({
    ...u,
    total_hours: Math.round(u.total_minutes / 60 * 100) / 100,
    estimated_salary: Math.round(u.total_minutes / 60 * hourlyRate * 100) / 100
  }));
  return jsonOk({ summary, logs, hourly_rate: hourlyRate });
});
...
app.put("/api/settings", async (c) => {
  ...
  for (const [key, value] of Object.entries(body)) {
    await db.insert(systemSettings).values({ key, value, updated_at: nowISO() }).onConflictDoUpdate({ target: systemSettings.key, set: { value, updated_at: nowISO() } });
  }
...
app.get("/api/inventory", async (c) => {
  ...
  const transactions = await db.select({
    ...
    user_name: users.name
  }).from(inventoryTransactions).innerJoin(users, eq(users.id, inventoryTransactions.user_id)).orderBy(desc(inventoryTransactions.created_at));
...
app.post("/api/inventory/transactions", async (c) => {
  ...
  await db.update(inventoryItems).set({ current_stock: sql`current_stock + ${delta}` }).where(eq(inventoryItems.id, body.item_id));
  return jsonOk({ id });
});
app.get("/api/audit-logs", async (c) => {
  ...
  let query = db.select({
    ...
    old_value: auditLogs.old_value,
    new_value: auditLogs.new_value,
    reason: auditLogs.reason,
    created_at: auditLogs.created_at,
    user_name: users.name
  }).from(auditLogs).innerJoin(users, eq(users.id, auditLogs.user_id)).orderBy(desc(auditLogs.created_at)).$dynamic();
...
// .wrangler/tmp/bundle-oBGfN9/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-oBGfN9/middleware-loader.entry.ts
